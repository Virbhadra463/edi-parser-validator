...
class EDIValidator:
    ...
    def validate(self, parsed: dict, tx_type: str) -> List[Dict[str, Any]]:
        ...
        detected = self.GUIDE_MAP.get(guide)
        if detected and detected != tx_type:
            errors.append(self._err(
                segment="ST", element="ST03",
                error_code="H000",
                severity="error",
                message=f"Transaction declared as {tx_type} but guide '{guide}' indicates {detected}.",
    ...
    def _validate_837(self, parsed: dict, tx_type: str) -> List[Dict[str, Any]]:
        ...
        for claim in claims:
                ...
                for dx in diagnoses:
                    ...
                    if qualifier not in ("ABK", "ABF", ""):
                        errors.append(self._err(
                            ...
                            error_code="H008",
                            severity="warning",
                            message=f"Claim '{claim_id}': diagnosis qualifier '{qualifier}' is non-standard.",
                            explanation=(
                                "HIPAA 5010 requires ICD-10 qualifier ABK (primary) or ABF (additional). "
                                "ICD-9 qualifiers like BK/BF are no longer accepted for 5010 transactions."
                            ),
                            suggestion="ABK" if qualifier in ("BK", "BF") else None,
                        ))
            ...
            if rnpi:
                if not rnpi.isdigit() or len(rnpi) != 10:
                    errors.append(self._err(
                        ...
                        error_code="H015",
                        severity="error",
                        message=f"Claim '{claim_id}': rendering provider NPI '{rnpi}' must be a 10-digit number.",
                        explanation="NPIs must be exactly 10 digits as per CMS standards.",
                        suggestion=None,
                    ))
                elif not self._luhn_check(rnpi):
                    errors.append(self._err(
                        segment="NM1", element="NM109",
                        error_code="H016",
                        severity="error",
                        message=f"Claim '{claim_id}': rendering provider NPI '{rnpi}' fails Luhn checksum.",
                        explanation="Rendering provider NPI (Loop 2310B) must pass the Luhn algorithm check.",
                        suggestion=None,
                    ))
            ...
            for sub in subscribers:
                sbr_filing = sub.get("sbr_claim_filing", "")
                FILING_MAP = {"MC": "MB", "MA": "MB", "CH": "ZZ"}
                expected = FILING_MAP.get(sbr_filing)
                if expected and filing_indicator and filing_indicator != expected:
                    errors.append(self._err(
